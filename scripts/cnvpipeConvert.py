...
cnvList_part = cnvList.loc[:, ['toolNum', 'AS', 'DS', 'GS', 'MS']]
cnvList_part_scale = (cnvList_part - cnvList_part.min()) / (cnvList_part.max() - cnvList_part.min())
cnvList_part_scale['DS'].fillna(1, inplace=True)
cnvList_part_scale['GS'].fillna(1, inplace=True)
cnvList_part_scale['MS'].fillna(1, inplace=True)
cnvList_part_scale.rename(columns={'toolNum':'toolNum_scale', 'AS':'AS_scale', 'DS':'DS_scale', 'GS':'GS_scale', 'MS':'MS_scale'}, inplace=True)
...
overall_score = new_cnvList['toolNum_scale'] + new_cnvList['AS_scale'] + new_cnvList['DS_scale'] + new_cnvList['GS_scale'] + new_cnvList['patho_score']*7 + new_cnvList['dosage_score'] + new_cnvList['MS_scale']
...
new_cnvList = new_cnvList.assign(overall_score=overall_score.round(3))
...
new_cnvList = new_cnvList.assign(priority=priority_label)
